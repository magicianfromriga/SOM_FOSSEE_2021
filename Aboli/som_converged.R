...
SOM <- function(x, input_grid) {
  ...
  for(ne in 1:n_epochs)
  {
    ...
    for (i in 1:n_iteration) # Looping through for training
    {
      ...
      new_radius <- decay_radius_function(initial_radius, ini_i, time_constant) # Decaying radius
      new_learning_rate <- max(decay_learning_rate(initial_learning_rate,ini_i, n_iteration), 0.01) # Decaying learning rate
      index_temp <- BMU(sample_input_row, input_grid) # Finding best matching unit for given input row
      index_new=c((as.integer(index_temp/rows)),(index_temp%%rows)) #Converting a 1D co-ordinate to a 2D co-ordinate for finding lateral distance on the map.
      print(index_new)
